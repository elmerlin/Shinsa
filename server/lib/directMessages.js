...
function sanitizeLinkSharePayload(linkShare) {
  ...
  return {
    ...
    postType: String(src.postType || src.post_type || '').trim().slice(0, 40),
    achievementBadgeName: String(src.achievementBadgeName || src.achievement_badge_name || '').trim().slice(0, 120),
    achievementSupportingCopy: String(src.achievementSupportingCopy || src.achievement_supporting_copy || '').trim().slice(0, 320),
    previewImage: sanitizeLinkShareAsset(src.previewImage || src.preview_image || ''),
    ...
    liveSessionType: String(src.liveSessionType || src.live_session_type || '').trim().slice(0, 24),
    isUnlisted: toBoolean(src.isUnlisted || src.is_unlisted),
    youtubeVideoId: String(src.youtubeVideoId || src.youtube_video_id || '').trim().slice(0, 24),
    ...
    bad: toInt(src.bad),
    miss: toInt(src.miss),
    replayUrl: sanitizeAbsoluteUrl(src.replayUrl || src.replay_url || src.replayEmbedUrl || src.replay_embed_url),
    replayVideoId: String(src.replayVideoId || src.replay_video_id || '').trim().slice(0, 24),
    replayStartSeconds: toInt(src.replayStartSeconds || src.replay_start_seconds),
    replayEndSeconds: toInt(src.replayEndSeconds || src.replay_end_seconds),
...
function normalizeConversationRow(row) {
  ...
  const theme = String(row.conversation_theme || '').trim();
