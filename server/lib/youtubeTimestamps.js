...
function normalizeChapterLines(text) {
  return String(text || '')
    .split(/\r?\n/)
...
function buildManagedYoutubeChaptersBlock(textOrLines) {
  const normalizedLines = Array.isArray(textOrLines)
    ? textOrLines.map((line) => String(line || '').trim()).filter(Boolean)
    : normalizeChapterLines(textOrLines);
...
function fitManagedYoutubeChaptersBlock(description, text) {
  const baseDescription = stripManagedYoutubeChaptersBlock(description);
  const chapterLines = normalizeChapterLines(text);

  if (!chapterLines.length) {
    const err = new Error('No YouTube chapters are available to publish');
    err.statusCode = 400;
    throw err;
  }

  if (baseDescription.length >= YOUTUBE_DESCRIPTION_MAX_LENGTH) {
    const err = new Error('The current YouTube description is already too long to append chapters');
    err.statusCode = 400;
    throw err;
  }

  const joinDescription = (block) => (baseDescription ? `${baseDescription}\n\n${block}` : block).trim();

  let keptLines = [...chapterLines];
  let block = buildManagedYoutubeChaptersBlock(keptLines);
  let nextDescription = joinDescription(block);

  while (keptLines.length > 0 && nextDescription.length > YOUTUBE_DESCRIPTION_MAX_LENGTH) {
    keptLines.pop();
    block = buildManagedYoutubeChaptersBlock(keptLines);
    nextDescription = joinDescription(block);
  }

  if (!keptLines.length) {
    const err = new Error('The current YouTube description leaves no room to publish chapter markers');
    err.statusCode = 400;
    throw err;
  }
  ...
  return {
    ...
    published_text: keptLines.join('\n'),
    truncated_chapter_count: Math.max(0, chapterLines.length - keptLines.length),
    total_chapter_count: chapterLines.length,
  };
}

function upsertManagedYoutubeChaptersBlock(description, text) {
  return fitManagedYoutubeChaptersBlock(description, text).description;
}
