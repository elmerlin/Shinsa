...
export function SingleWeeklyChallengePlayPage() {
  const { id } = useParams();
  const [item, setItem] = useState(null);
  const [loading, setLoading] = useState(true);
  const [jacketLookup, setJacketLookup] = useState({});
  ...
  if (loading) return <div className="max-w-2xl mx-auto px-4 py-12 text-center text-gray-500">Loading...</div>;
  if (!item) return <div className="max-w-2xl mx-auto px-4 py-12 text-center text-gray-500">Weekly challenge play not found</div>;
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="card">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <Link to={`/profile/${item.user_id}`} className="font-display font-bold text-white hover:text-piu-accent">
              {flag} {item.username}
            </Link>
            <span className="text-gray-400 ml-1.5">played weekly challenges!</span>
          </div>
          <Link to={`/weekly-challenges?week=${weekKey}`} className="text-[9px] px-2 py-1 rounded bg-purple-500/15 text-purple-300 border border-purple-500/25 font-display font-bold">
            {weekKey}
          </Link>
        </div>
