...
function ResourcePill({ resource, value, tone = 'neutral', capacity, onClick }) {
  const tones = {
    neutral: 'cf-pill',
  ...
  const className = `${tones[tone] || tones.neutral} shrink-0`;
  const style = { gap: 2, padding: '1px 5px' };
  const inner = (
    <>
...
export default function PetWorldHUD({
  ...
  const resourcePills = resourceValues.map(({ key, value, capacity }) => (
    <ResourcePill
      ...
      onClick={onExplain && key === 'food' ? () => onExplain('food') : undefined}
    />
  ));
