...
const SIZE_STYLES = {
  xs: {
    ...
    fallback: 'text-xs',
    badge: '-right-0.5 -bottom-0.5 min-w-[16px] h-[16px] px-0.5 text-[8px] rounded-full',
  },
  sm: {
    frame: 'w-[42px] h-[24px] rounded-md',
    fallback: 'text-xs',
    badge: '-right-0.5 -bottom-0.5 min-w-[17px] h-[17px] px-0.5 text-[8px] rounded-full',
  },
  md: {
    frame: 'w-[50px] h-[28px] rounded-lg',
    fallback: 'text-xs',
    badge: '-right-0.5 -bottom-0.5 min-w-[18px] h-[18px] px-0.5 text-[8px] rounded-full',
  },
  wide: {
    frame: 'w-[66px] h-[38px] rounded-lg',
    fallback: 'text-[10px]',
    badge: '-right-0.5 -bottom-0.5 min-w-[19px] h-[19px] px-0.5 text-[9px] rounded-full',
  },
};
...
export default function PiuChartJacket({
  ...
  return (
    <div className={`relative shrink-0 ${className}`}>
      ...
      {withBadge ? (
        <span
          className={`absolute inline-flex items-center justify-center border font-display font-black leading-none tracking-[-0.04em] text-white ${styles.badge} ${getBadgeTone(mode)}`}
        >
          {badgeLabel}
        </span>
      ) : null}
