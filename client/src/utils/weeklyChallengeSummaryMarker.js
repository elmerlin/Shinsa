...
export function serializeWcSummaryMarker(summary) {
...
export function parseWcSummaryMarker(content) {
...
export function splitWcSummaryContent(content) {
  const raw = String(content || '');
  const match = raw.match(WC_SUMMARY_MARKER_REGEX);
  if (!match) {
  ...
  const summary = parseWcSummaryMarker(raw);
  const text = raw
    .replace(match[0], '')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
